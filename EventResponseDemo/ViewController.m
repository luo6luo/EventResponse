...
- (void)viewDidLoad {
    ...
    /*
     ...
     == iOS9中新增属性 ==
     9、UITouchType           --> 触摸类型（直接手指触摸：Direct、间接触摸：Indirect、触摸笔触摸：Stylus）
     ...
     2、UIApplication遵循先进先出的原则，将队列中先进的时间发送给主窗口（keyWindow）
     ...
     1、先判断view是否能接收触摸事件
     2、触摸点是否在view身上
     3、从后向前遍历子控件，然后重复上面两步，找到则返回合适的子控件
     4、没找到合适的子控件，则自己就是合适处理的
     
     
     点击方法的内部实现方法：
        // 时间一传递给控件，就会调用下列方法，返回最合适接收时间的UIView
